    ...
    for start in tqdm(range(0, num_samples, batch_size), desc="Processing batches"):
        ...
        def hook_fn(activation: torch.Tensor, hook) -> torch.Tensor:
            ...
            if not use_sae:
                ...
                activ = activation[:, -1, policemantic_neurons]
                
            all_dampen = []
            for chunk in torch.split(activ, 20, dim=0):
                # chunk: [b, D]
            ...
            all_dampen = torch.cat(all_dampen, dim=1)  # [D, b]
            all_dampen = all_dampen.T  # [b, D]
            if not use_sae:
                activation[:, -1, :] *= all_dampen  # apply dampening to all neurons
            else:
                activation[:, -1, policemantic_neurons] *= all_dampen
