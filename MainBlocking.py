...
from utility import tokenize_prompt_dataset, is_match, tokenize_prompt_word_dataset, convert_few_nerd_to_dict
import pandas as pd


device = "cuda"
combination_type = "last_token"  # Options: "last_token", "all_tokens_max"
data_name = "emotions"  # Change this to the dataset you want to use, e.g., "ag_news", "imdb", etc.
...
for blocking_approach in ["range", "adaptive", "full"]:
    
    print(f"Blocking approach: {blocking_approach}")
    ...
    for class_neuron in classes:
        ...
        for key in process_keys:
            data = torch.tensor(frequencies[key], device=device)
            mu_val = data.mean()
            std_val = data.std(unbiased=True)
            mu_std_dict[key] = (mu_val, std_val)

        sorted_cls = sorted(mu_std_dict.items(), key=lambda item: item[1][0], reverse=True)
        top = int(len(sorted_cls) * importance_threshold)
        top_keys = [key for key, _ in sorted_cls[:top]]
        
        all_mus = torch.stack([mu_std_dict[key][0] for key in top_keys])
        all_stds = torch.stack([mu_std_dict[key][1] for key in top_keys])

        all_max = all_mus + tau * all_stds
        all_min = all_mus - tau * all_stds
        ...
        for start in tqdm(range(0, num_samples, batch_size), desc="Processing batches"):
            ...
            with torch.no_grad():
                if not use_sae:
                    logits = finetune_model(batch_input_ids,
            ...
            def hook_fn(activation: torch.Tensor, hook) -> torch.Tensor:
                if blocking_approach == "range":
                    activ = activation[:, -1, top_keys]
                    mask = (activ > all_min) & (activ < all_max)
                    activation[:, -1, top_keys] = activ.masked_fill(mask, 0)
                elif blocking_approach == "full":
                    activation[:, -1, top_keys] = 0
                else: # adaptive blocking
                    activ = activation[:, -1, top_keys]
                    mask = (activ > all_min) & (activ < all_max)
                    dampening = betha * torch.abs((activ - all_mus)) / (tau * all_stds)
                    dampening = dampening.to(activation.dtype)
                    activation[:, -1, top_keys] = torch.where(mask, activ * dampening, activ)
            ...
            with torch.no_grad():
                if not use_sae:
                    blocked_logits = finetune_model.run_with_hooks(batch_input_ids,
                                                                    attention_mask=batch_attention_mask,
                                                                    return_type="logits",
        ...
        if report_per_class:
            print("--------------------------------------------------------------------")
            for class_name in classes:
                if class_name != class_neuron:
                    class_index = classes.index(class_name)
                    print(f"Class: {class_name}")
                    print(f"Accuracy drop: {accuracy_drops[class_index]}")
                    print(f"Confidence drop average: {confidence_drops[class_index]}")
                    print(f"Ranking drop average: {ranking_drops[class_index]}")
                    print("--------------------------------------------------------------------")
